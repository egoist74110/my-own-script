...
def check_git_clean(cwd: Path) -> tuple[bool, str]:
    cp = _run(["git", "status", "--porcelain"], cwd, timeout=5)
    if cp.returncode != 0:
...
def get_update_status(cwd: Path, branch: str = "main") -> UpdateStatus:
    ...
    cp0 = _run(["git", "rev-parse", "--is-inside-work-tree"], cwd, timeout=5)
...
def pip_sync(cwd: Path) -> str:
    ...
    cp = _run([sys.executable, "-m", "pip", "install", "-r", str(req)], cwd, timeout=600)
    if cp.returncode != 0:
